...
// When the DB is connected, queries real models; falls back to DEMO_* constants when offline.

const isDBConnected = () => require('mongoose').connection.readyState === 1;

// ─── Demo datasets (offline / no-data fallback) ───────────────────────────────
...
exports.getBeds = async (req, res) => {
    try {
        if (!isDBConnected()) return res.json({ success: true, data: DEMO_BEDS });
        ...
        if (allBeds.length === 0) return res.json({ success: true, data: DEMO_BEDS });
        ...
        // Aggregate by ward type for the summary table
        const wardMap = {};
        ...
        res.json({
            ...
            data: {
                ...
                beds,
                wards,
                adtRequests: DEMO_BEDS.adtRequests, // ADT requests come from adtController
            },
        });
...
exports.getEmergency = async (req, res) => {
    try {
        if (!isDBConnected()) return res.json({ success: true, data: DEMO_EMERGENCY });
        ...
        if (erAppts.length === 0) return res.json({ success: true, data: DEMO_EMERGENCY });
        ...
        res.json({
            success: true,
            data: {
                stats: { critical, stable, triageWaiting: patients.length, averageWaitMins: 20, enRoute: 0 },
                patients,
            },
        });
    } catch (err) { res.status(500).json({ success: false, error: err.message }); }
...
exports.getNursing = async (req, res) => {
    try {
        if (!isDBConnected()) return res.json({ success: true, data: DEMO_NURSING });
        ...
        if (activeAppts.length === 0) return res.json({ success: true, data: DEMO_NURSING });
...
exports.getOT = async (req, res) => {
    try {
        if (!isDBConnected()) return res.json({ success: true, data: DEMO_OT });
        ...
        if (surgeries.length === 0) return res.json({ success: true, data: DEMO_OT });
        ...
        res.json({
            success: true,
            data: {
                stats: { todaySurgeries: surgeries.length, runningNow: surgeries.filter(s => s.status === 'In_Consultation').length, delayed: 0, availableORs: 5 - surgeries.length },
                procedures,
                schedule: [],
            },
        });
...
exports.getEMS = async (req, res) => {
    // EMS/ambulance dispatch data comes from an external CAD system in production.
    // The DEMO_EMS dataset represents a realistic operational view until that integration lands.
    try {
