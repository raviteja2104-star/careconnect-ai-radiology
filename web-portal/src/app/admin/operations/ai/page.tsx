...
export default function SmartQueueAIOptimiser() {
  ...
  const { data: predictionsRes } = useQuery({
    ...
    queryFn: () => fetch('http://localhost:5000/api/operations/predictions').then(res => res.json()),
  ...
  const { data: recommendationsRes } = useQuery({
    ...
    queryFn: () => fetch('http://localhost:5000/api/operations/recommendations').then(res => res.json())
  });
