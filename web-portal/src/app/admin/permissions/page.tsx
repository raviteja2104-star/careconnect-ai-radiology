...
import {
    Shield, Users, ChevronDown, ChevronUp, Plus, Trash2, Search,
    CheckCircle2, XCircle, AlertCircle, RefreshCw, Lock,
} from 'lucide-react';
import {
...
type ActiveTab = 'users' | 'roles' | 'catalogue';
...
function PermissionsPageInner({
    ...
    const assignRoleMutation = useMutation({
        mutationFn: ({ userId, roleId }: { userId: string; roleId: string }) =>
            apiFetch('/api/rbac/users/assign-role', {
                method: 'POST',
    ...
    const revokeRoleMutation = useMutation({
        mutationFn: ({ userId, roleId }: { userId: string; roleId: string }) =>
            apiFetch('/api/rbac/users/revoke-role', {
                method: 'POST',
                body: JSON.stringify({ userId, roleId }),
            }),
        onSuccess: () => qc.invalidateQueries({ queryKey: ['rbac', 'users'] }),
    ...
    // Group permissions by workspace prefix
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {role.workspaces.map(w => (
                                            <Badge key={w} tone="brand" className="text-xs">{w}</Badge>
                                        ))}
                                    </div>
                                </div>
                                <div>
