...
/**
 * Inner component — uses useSearchParams so it must live inside a Suspense
 * boundary (provided by LoginChooserPage below).
 */
function PortalGrid() {
    ...
    return (
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {LOGIN_PORTALS.map((portal, i) => (
                ...
                >
                    <Link
                        href={`/login/${portal.id}${next ? `?next=${encodeURIComponent(next)}` : ''}`}
    ...
    return (
        <div className="flex min-h-screen flex-col items-center justify-center gradient-surface px-4 py-12">
            ...
            >
                ...
                <React.Suspense fallback={<div className="mt-10 h-64 animate-pulse rounded-2xl bg-muted/40" />}>
                    <PortalGrid />
                </React.Suspense>
