...
export default function AppointmentsPage() {
  ...
  const todayLabel = selectedDate.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' });
        ...
        <div className="px-8 pt-7 pb-4">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">{todayLabel}</p>
            </div>
            <div className="flex gap-3">
              <div className="flex items-center gap-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm">
                <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-l-lg transition-colors"><ChevronLeft className="w-4 h-4 text-slate-500" /></button>
                <span className="px-3 text-sm font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">Today</span>
                <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-r-lg transition-colors"><ChevronRight className="w-4 h-4 text-slate-500" /></button>
              </div>
        ...
        <div className="px-8 pb-8 flex-1">
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800">
              <span className="text-sm text-slate-500">{filtered.length} appointments</span>
            </div>
                  ...
                  <div key={appt.id} className="flex items-center gap-4 px-5 py-4 hover:bg-indigo-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                      {appt.type === 'Telemedicine' && appt.status !== 'Completed' && (
                        <button className="p-1.5 bg-purple-50 hover:bg-purple-100 dark:bg-purple-900/30 text-purple-600 rounded-lg" title="Join Video"><Video className="w-3.5 h-3.5" /></button>
                      )}
